...
tar -czf cronet_native-$pkgver-linux.tar.gz -C chromium-$pkgver/out/release/cronet .
...
archive_override(
    ...
    url = "https://github.com/zifeitong/libcronet/releases/download/${pkgver}/cronet_native-${pkgver}-linux.tar.gz",
    sha256 = "`sha256sum cronet_native-${pkgver}-linux.tar.gz | awk '{print $1}'`",
)
EOF
